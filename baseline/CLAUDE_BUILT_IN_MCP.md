...
| Version | Changes |
|---------|---------|
| v1.11847.5 | Registration function `registerInternalMcpServer:KqA` (was `HHA` in v1.9659.1; intervening releases v1.9659.2-v1.11187.4 not row-logged here - roster was unchanged throughout). **Server roster and tool sets unchanged**: same internal set (mcp-registry, plugins, skills, cowork-onboarding, radar, "Claude Preview", dev-debug, ccd_session_mgmt, ccd_directory, ccd_session, Framebuffer, Window Halo) plus renderer-facing ("Claude in Chrome", terminal, visualize); no new or removed servers. `office` still an IPC bridge (not an MCP server), `fix_office_addin_linux` applies. 3 new **feature flags** this release (`coworkRemoteSessionSpaces`, `coworkBranchSession`, `epitaxyMcpApps`) are renderer feature toggles, not MCP servers. `epitaxyMcpApps` (flag `3516166472`) gates MCP *apps* inside epitaxy/SSH-remote sessions - server-gated, no new built-in MCP server registered. Cowork protocol unchanged from the Desktop side (all `fix_cowork_*` patches apply); `claude-cowork-service` not checked out this session. All 45 patches applied. |
