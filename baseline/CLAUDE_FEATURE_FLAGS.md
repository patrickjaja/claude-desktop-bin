...
Reference documentation for the feature flag system in Claude Desktop's Electron app, to aid patch maintenance. The architecture/prose below was first written against v1.8555.2; minified names drift every release, so the **version history table at the bottom is the authoritative record of renames** (current through v1.20186.1). Always cross-check a specific name there before trusting it.
...
**Merger change in v1.20186.1:** async override object grew 5 -> 6 keys — new `launch` override (`Oae()` = `At("2976814254")||!1`, the claudePreview/launch-server flag, already force-ON via enable_local_agent_mode Patch 3j; no new work needed).
