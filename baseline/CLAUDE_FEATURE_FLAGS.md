...
| `1496676413` | ~~SSH remote MCP/plugin passthrough (`adjustSdkOptions`)~~ **UPSTREAMED in v1.18286.0** (see below) | No — regression guard |
...
| `975112542` | Cowork memory remote sync (`canSyncCoworkMemoryRemotely()`) | No |
| `1496676413` | ~~SSH plugin/MCP stripping — gates plugin and MCP forwarding to SSH sessions~~ **UPSTREAMED in v1.18286.0** (see below) | No — regression guard |
...
| Flag ID | Purpose | Patched? |
|---------|---------|----------|
| `1496676413` | ~~SSH session plugins/MCP forwarding — gates plugin and MCP server forwarding to SSH remote sessions (6 call sites in session start, spawn, and MCP resolution)~~ **UPSTREAMED in v1.18286.0**: flag literal removed from the bundle entirely; every former call site (`resolveSshControllerForMcp()`, `spawnClaudeCodeProcess` spawn-function arg, the `adjustSdkOptions()` plugin/MCP-stripping method) now runs unconditionally — SSH remote plugin/MCP forwarding is default-on | No — `enable_local_agent_mode.nim` Patch 3n converted to a regression guard asserting `resolveSshControllerForMcp()` is unconditional |
...
| v1.18286.0 | not re-audited | not re-audited | not re-audited | **Bump v1.17377.2 -> v1.18286.0, scoped audit only (issue #176 — fixing a failed auto-release, not a full flag sweep; v1.17282.0 -> v1.17377.x rows were never backfilled either).** Flag reader renamed `et()`->`rt()`. **GrowthBook flag `1496676413` (SSH remote plugin/MCP passthrough) fully UPSTREAMED**: the literal string is gone from the bundle everywhere, and every call site that used to gate on it now runs unconditionally — `resolveSshControllerForMcp(e){if(!(!e||!et("1496676413")))return Jc(e)}` -> `resolveSshControllerForMcp(e){if(e)return jB(e)}`; `spawnClaudeCodeProcess=o.createSpawnFunction(e.stderr,et("1496676413"),s,c,a)` -> same call with the flag arg dropped; the `adjustSdkOptions(e){et("1496676413")||(delete e.plugins,delete e.mcpServers)}` method that used to strip plugins/MCP off SSH sessions is gone from the SSH backend class entirely. `enable_local_agent_mode.nim` Patch 3n converted from a force-ON to a regression guard (asserts `resolveSshControllerForMcp` is unconditional). Verified via direct diff against a freshly downloaded v1.17377.2 `.deb` (still hosted on the apt repo at audit time), not guesswork. Three more re-minify-only pattern fixes (no behavior change, no flag involved): `fix_buddy_ble_linux` (new intermediate `hardwareBuddyEnabled` workspace-pref clause inserted before the GrowthBook flag read), `fix_computer_use_linux` Patch 6/11 (the CU tool's `isEnabled` gate moved from a standalone function call to an inline `A.sessionType==="ccd"?wS():bue()` ternary in the object literal) and Patch 5b/12 (a `setTimeout` watchdog inserted between the screenshot `AbortController` and its options object; the `chicagoEnabled`-respecting enablement check `EO()` refactored from a ternary into an early-return form with a test-override var, renamed `wS()`). All 47 patches apply clean post-fix. |
