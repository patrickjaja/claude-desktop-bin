...
| Flag ID | Purpose | Patched? |
|---------|---------|----------|
| `3516166472` | `epitaxyMcpApps` async gate - MCP apps inside epitaxy (SSH remote) sessions; merger helper `zQt()` (5s delay + `ctt()` prereq + this flag) | No |
...
**New static features in v1.11847.5** (see version history row): `coworkRemoteSessionSpaces` (always supported), `coworkBranchSession` (always supported), `epitaxyMcpApps` (static unavailable + async override via `3516166472`). None platform-gated; `enable_local_agent_mode.nim` override list unchanged.
...
| v1.11847.5 | `Rw()` | `PBA` | `OS()` | **Version bump v1.11187.4 -> v1.11847.5 (~660 builds, full re-minify).** **3 new static features:** `coworkRemoteSessionSpaces:Bu` (always supported, no gate), `coworkBranchSession:Bu` (always supported, no gate), `epitaxyMcpApps` (static `{status:"unavailable"}` + 5th async override) -> **36 static + `louderPenguin` async-only = 37 total**; no features removed. Async merger now **5-way** `Promise.all`: `PBA=async()=>{const[A,e,t,i,r]=await Promise.all([Xur(),JNA(()=>lt("123929380")),JNA(()=>lt("2940196192")),JNA(()=>lt("3732274605")),zQt(()=>lt("3516166472"))]);return{...Rw(),louderPenguin:A,coworkKappa:e,coworkArtifacts:t,markTaskComplete:i,epitaxyMcpApps:r}}` - `epitaxyMcpApps` helper `zQt()` = `await e4A(5e3)` (5s) + `await ctt()` prereq + flag `3516166472` (server-gated, no platform gate). Function renames: registry `Dw()`->`Rw()`, async merger `SBA`->`PBA`, dev-gate `MS()`->`OS()` (`function OS(A){return oA.app.isPackaged?{status:"unavailable"}:A()}`, electron var `sA`->`oA`; 2nd dev-gate `xEr()`->`xur()` for `builtinMcpPresets`), `louderPenguin` async helper `XEr()`->`Xur()` (still `darwin\|\|win32` gate + `await IR(),lt("4116586025")`), cowork helper `mNA()`->`JNA()`, supported constant `_d`->`Bu`; GrowthBook bool reader `lt()` unchanged. **4 new boolean GrowthBook flags** (verified absent from patched-v1.11187.4 + this doc): `3516166472` (epitaxyMcpApps), `1997559319` (onUserDialog refusal-fallback dialogs), `2724639973` (session governor eviction), `3807767338` (policy-limits seeding); precise add/remove delta not reliable (only local prior bundle is patched). All 3 new feature keys present in the Zod `.partial()` schema. `enable_local_agent_mode.nim` 12-flag override list unchanged (25/25 sub-patches; **semantically verified** the override block lands after all 5 async merger props - `...,markTaskComplete:i,epitaxyMcpApps:r,` then the 12 overrides - so Code tab + Cowork still win on Linux); `epitaxyMcpApps` deliberately not force-enabled (new, server-gated, no platform gate - unavailable on all OSes until the flag flips). 1 patch fixed (`fix_claude_code` getStatus - upstream added `\|\|await this.getHostPreseedInPlacePath()` disjunct), all 45 applied. |
