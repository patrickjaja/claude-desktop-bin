...
Reference documentation for the feature flag system in Claude Desktop's Electron app. This documents v1.6608.0 internals to aid patch maintenance.
...
2. **`woA` (async merger)** - Spreads `pw()`, adds `louderPenguin` + `coworkKappa` + `coworkArtifacts` + `markTaskComplete` as async overrides (4 total; `operon` removed)
3. **IPC handler** - Calls merger, validates against schema, sends to renderer

`pt()` flag reader (was `Jt()`), `fM()` listener (was `kM()`), `ew()` single-value reader (was `lp()`), `Bn()` multi-key reader (was `dn()`).
...
| 6 | `louderPenguin` | `await Nvi()` in woA only | **async override** in woA; platform gate (darwin/win32) + GrowthBook `4116586025` | **Code tab** |
...
| 15 | `desktopTopBar` | `q_r()` | **None** (always supported) | Desktop top bar |
| 16 | `ccdPlugins` | `saA` (constant) | **None** (always supported) | CCD Plugins UI (Add plugins, Browse plugins) |
| 17 | `computerUse` | `$_r()` | Set-based check on `process.platform` | Computer use feature flag (**patched for Linux** via Set modification) |
| 18 | `coworkKappa` | static: `Z_r()` (unavailable) + async in woA | Depends on yukonSilver + GrowthBook `123929380` | Memory consolidation — `consolidate-memory` skill |
| 19 | `coworkArtifacts` | static: `X_r()` (unavailable) + async in woA | Depends on yukonSilver + GrowthBook `2940196192` | **Cowork artifacts** — artifact rendering in cowork sessions |
| 20 | `markTaskComplete` | static: `ARr()` (unavailable) + async in woA | Depends on yukonSilver + GrowthBook `3732274605` | **Task completion** — mark tasks as done |
| 21 | `framebufferPreview` | `ivr()` | **MW() production gate** + GrowthBook `1928275548` | VNC framebuffer preview (dev-gated) |
| 22 | `iosSimulator` | `MW(rvr)` | **MW() production gate** + macOS-only | iOS Simulator integration (dev-gated + macOS-only) |
| 23 | `androidEmulator` | `MW(...)` | **MW() production gate** + macOS-only | Android Emulator integration (dev-gated + macOS-only; removed in v1.6259.1, re-added v1.6608.0) |
| 24 | `grandPrix` | `XTi()` | darwin-only, checks connected device pairs | Device pairing (macOS-only; removed in v1.6259.1, re-added v1.6608.0) |
| 25-27 | *(async-only: `louderPenguin`, `coworkKappa`, `coworkArtifacts`, `markTaskComplete`)* | See rows 6, 18-20 | async overrides in woA | See respective rows |
...
**Features gated by MW():** `plushRaccoon`, `quietPenguin`, `wakeScheduler`, `framebufferPreview`, `iosSimulator`, `androidEmulator`

Note: `louderPenguin` is no longer in pw() at all. It exists only in woA as `await Nvi()`, which has its own platform gate (darwin/win32 only) + server feature flag check via GrowthBook `4116586025`. `operon` has been completely removed in v1.6608.0. `coworkKappa`, `coworkArtifacts`, and `markTaskComplete` are async-only: static returns unavailable, async checks yukonSilver + respective GrowthBook flags. `chillingSlothPool` is GrowthBook-gated directly in the static registry.

This is why patching the inner functions alone is insufficient - MW() never calls them in packaged builds.

## The Three Layers

### Layer 1: pw() - Static Registry
...
function pw(){
  return{
    ...
    framebufferPreview:MW(...),        // dev-gated + GrowthBook 1928275548
    iosSimulator:MW(...),              // dev-gated + macOS-only
    androidEmulator:MW(...),           // dev-gated + macOS-only (new in v1.6608.0)
    grandPrix:...,                     // macOS-only, checks device pairs via XTi() (new in v1.6608.0)
  }
}
```
...
### Layer 2: woA - Async Merger

```javascript
const woA=async()=>{
  const[e,A,t,i]=await Promise.all([Nvi(),D1A(()=>pt("123929380")),D1A(()=>pt("2940196192")),D1A(()=>pt("3732274605"))]);
  return{...pw(),louderPenguin:e,coworkKappa:A,coworkArtifacts:t,markTaskComplete:i}
};
```

Uses `Promise.all` to parallelize louderPenguin (`Nvi()`), coworkKappa (`D1A()+pt("123929380")`), coworkArtifacts (`D1A()+pt("2940196192")`), and markTaskComplete (`D1A()+pt("3732274605")`) async checks. Spreads `pw()` then adds the four as async overrides. `Nvi()` checks platform (darwin/win32) then checks server feature flag `4116586025`. The `D1A()` helper checks yukonSilver first, waits 5 seconds, then checks the respective GrowthBook flag. **`operon` was removed in v1.6608.0** — no longer has a static entry or async override.
...
**Because spread applies earlier properties first, later properties win.** This is how our Linux patch works - we append overrides after the last async property so they take precedence over gate-blocked values from `...pw()`.
...
## GrowthBook Flag Catalog (v1.6608.0)
...
### Object/Value Flags (Bn())

`ew()` reads single-value flags; `Bn()` reads multi-key object/value flags.
...
**Patch 3 - woA merger override:** Append to the `woA` return object:
```javascript
...
This bypasses the MW() gate by overriding at the merger level (12 total overrides). The spread order ensures our values win:
```
...pw()           -> quietPenguin: {status:"unavailable"}  (from MW)
...
- `chillingSlothFeat`, `chillingSlothLocal`, `yukonSilver`, `yukonSilverGems`, and `ccdPlugins` are all overridden to `{status:"supported"}` in the woA merger
...
| `framebufferPreview` | Dev-only (MW() gate) |
...
1. Check if `woA` structure changed (new features added, order changed)
2. Check if MW()-wrapped features changed
