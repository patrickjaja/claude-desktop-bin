...
def patch_tray_dbus(filepath):
    ...
    if tray_func:
        ...
        if old_func in content and async_check not in content:
            ...
            print("  [INFO] async conversion: already async")
        else:
            print("  [FAIL] async conversion: function pattern not found")
            failed = True
    ...
    if tray_func and first_const:
        ...
        if mutex_check not in content:
            ...
            mutex_prefix = b"async function " + tray_func + b"(){if(" + tray_func + b"._running)return;" + tray_func + b"._running=true;setTimeout(()=>" + tray_func + b"._running=false,500);"
            if old_start in content:
                content = content.replace(old_start, mutex_prefix)
                print("  [OK] mutex guard: added")
            else:
                print("  [FAIL] mutex guard: insertion point not found")
                failed = True
        else:
            print("  [INFO] mutex guard: already present")
    ...
    if tray_var:
        ...
        if old_destroy in content and new_destroy not in content:
            ...
            print("  [INFO] DBus cleanup delay: already present")
        else:
            print("  [FAIL] DBus cleanup delay: destroy pattern not found")
            failed = True
