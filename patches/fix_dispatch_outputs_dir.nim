...
proc apply*(input: string): string =
  ...
  if alreadyA:
    ...
    # The openPath call may wrap the dir var in a path-translation function (e.g. Tc(r)).
    let patternA =
    ...
    if m.isSome:
      ...
      let wrapFn = match.captures[6] # optional path wrapper (e.g. "Tc"), may be empty

      # Build openPath arg: either wrap(_td) or plain _td
      let openPathArg =
        if wrapFn.len > 0:
          wrapFn & "(_td)"
        else:
      ...
      let replacement =
        ...
        "=await " & E & ".shell.openPath(" & openPathArg & ");" & sv & "&&" & L &
