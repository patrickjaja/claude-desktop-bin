...
proc apply*(input: string): string =
  ...
  if alreadyA:
    ...
    let patternA = re"""async openOutputsDir\(([\w$]+)\)\{const ([\w$]+)=([\w$]+)\.getOutputsDir\(\1\);([\w$]+)\.info\(`LocalAgentModeSessions\.openOutputsDir: sessionId=\$\{\1\}, outputsDir=\$\{\2\}`\);const ([\w$]+)=await ([\w$]+)\.shell\.openPath\(\2\);\5&&\4\.error\(`Failed to open outputs directory: \$\{\2\}, error: \$\{\5\}`\)\}"""
    ...
    if m.isSome:
      ...
      let replacement =
        ...
        "}" &
        "}catch(_e){}" &
        "const " & sv & "=await " & E & ".shell.openPath(_td);" &
        sv & "&&" & L & ".error(`Failed to open outputs directory: ${_td}, error: ${" & sv & "}`)" &
        "}"

      result = result[0 ..< match.matchBounds.a] & replacement & result[match.matchBounds.b + 1 .. ^1]
  ...
  if patchesApplied < EXPECTED_PATCHES:
    raise newException(ValueError, &"fix_dispatch_outputs_dir: Only {patchesApplied}/{EXPECTED_PATCHES} patches applied")
