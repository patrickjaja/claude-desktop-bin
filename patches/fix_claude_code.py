...
def patch_claude_code(filepath):
    ...
    def platform_replacement(m):
        ...
        linux_check = b'if(process.platform==="linux")return ' + arch_var + b'==="arm64"?"linux-arm64":"linux-x64";'
    ...
    if count1 >= 1:
        ...
        print("  [OK] getHostPlatform(): already patched")
    else:
        print("  [FAIL] getHostPlatform(): 0 matches, expected >= 1")
        failed = True
    ...
    linux_binary_check = (
        b'if(process.platform==="linux"){try{const fs=require("fs");'
    ...
    if count2 >= 1:
        ...
        print("  [OK] getBinaryPathIfReady(): already patched")
    else:
        print("  [FAIL] getBinaryPathIfReady(): 0 matches, expected >= 1")
        failed = True
    ...
    def status_replacement(m):
        ...
        return (
            b'async getStatus(){if(process.platform==="linux"){try{const fs=require("fs");'
            ...
            b"return "
            + enum_name
            + b".NotInstalled}catch(err){return "
            + enum_name
            + b".NotInstalled}}if(await this.getLocalBinaryPath())return "
            + enum_name
            + b".Ready;const "
            + var_name
            + b"=this.getHostTarget();if(this.preparingPromise)return "
            + enum_name
            + b".Updating;if(await this.binaryExistsForTarget("
            + var_name
            + b",this.requiredVersion))"
        )
    ...
    if count3 >= 1:
        ...
        print("  [OK] getStatus(): already patched")
    else:
        print("  [FAIL] getStatus(): 0 matches, expected >= 1")
        failed = True
