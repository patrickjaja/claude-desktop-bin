...
def patch_window_bounds(filepath):
    ...
    if bounds_marker in content:
        print("  [INFO] Window bounds fix already injected")
        ...
        def main_win_replacement(m):
            ...
            return prefix + win_var + b"=new " + electron_var + b".BrowserWindow(" + param_var + b")," + setup_call + b"," + iife + b"," + win_var + b"}"
        ...
        if count > 0:
            ...
            print("  [FAIL] Main window factory pattern not matched")
    ...
    if re.search(qe_blur_check, content):
        print("  [INFO] Quick Entry blur already applied")
        ...
        def qe_hide_replacement(m):
            ...
            return func_decl + guard_call + b"||(" + win_var + b".blur()," + win_var + hide_call + b")}"
