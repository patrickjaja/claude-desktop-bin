...
def patch_local_agent_mode(filepath):
    ...
    print("=== Patch: enable_local_agent_mode ===")
    ...
    def header_replacement(m):
        ...
        os_mod = m.group(4)  # Ma
        ver_var = m.group(6)  # r
        return m.group(1) + plat_var + m.group(3) + b'process.platform==="linux"?"darwin":' + os_mod + m.group(5) + ver_var + m.group(7)
    ...
    def ua_replacement2(m):
        ...
        return (
            ...
            + b"="
            + var
            + b'.replace(/X11; Linux [^)]+/g,"Macintosh; Intel Mac OS X 10_15_7")}'
            + m.group(6)
            + var
            + m.group(7)
        )
    ...
    if os.path.exists(mainview_path):
        ...
        def mv_replacement(m):
            proc_var = m.group(2)
            return m.group(1) + proc_var + m.group(3) + b'if(process.platform==="linux"){' + proc_var + b'.platform="win32"}'
