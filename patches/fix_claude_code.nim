...
proc apply*(input: string): string =
  ...
  if m1.isSome:
    ...
    let replacement = m.captures[0] & win32Return & ";" & linuxCheck & "throw new Error("
    result = result[0 ..< m.matchBounds.a] & replacement & result[m.matchBounds.b + 1 .. ^1]
    echo "  [OK] getHostPlatform(): 1 match(es)"
  elif "getHostPlatform(){" in result and "if(process.platform===\"linux\")return" in result and "\"linux-x64\"" in result:
  ...
  if m3.isSome:
    ...
    let replacement =
      ...
      ".Updating;if(await this.binaryExistsForTarget(" & varName & ",this.requiredVersion))"
    result = result[0 ..< m.matchBounds.a] & replacement & result[m.matchBounds.b + 1 .. ^1]
