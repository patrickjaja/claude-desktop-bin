...
proc apply*(input: string): string =
  ...
  let alreadyA = "process.platform===\"linux\"||" in result and "\"2358734848\"" in result
  if alreadyA:
    echo "  [OK] Buddy flag: already patched (skipped)"
    patchesApplied += 1
  else:
    let flagPattern = re2"(const [\w$]+=""2358734848"",)([\w$]+=\(\)=>)([\w$]+\([\w$]+\))"
    var countFlag = result.replaceFirst(flagPattern, proc(m: RegexMatch2, s: string): string =
  ...
  if alreadyB:
    ...
    var countStub = result.replaceFirst(pattern, proc(m: RegexMatch2, s: string): string =
      s[m.group(0)] & stubJs
    )
    if countStub >= 1:
      ...
      raise newException(ValueError, "fix_buddy_ble_linux: app.on(\"ready\") pattern not found")

  if patchesApplied < EXPECTED_PATCHES:
    raise newException(ValueError, &"fix_buddy_ble_linux: Only {patchesApplied}/{EXPECTED_PATCHES} patches applied")
