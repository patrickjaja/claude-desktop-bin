...
def patch_native_frame(filepath):
    ...
    if has_quick_entry:
        print("  [OK] Quick Entry pattern found (will preserve)")
    else:
        print("  [INFO] Quick Entry pattern not found (may be already patched)")
    ...
    if count > 0:
        ...
        print("  [INFO] No frame:!1 found outside Quick Entry (main window uses native frames by default)")

    # Step 4: Restore Quick Entry frame setting
    if has_quick_entry:
        content = content.replace(b"transparent:!0," + marker, quick_entry_pattern)
        print("  [OK] Restored Quick Entry frame:!1 (transparent)")
