...
```bash
wget https://github.com/patrickjaja/claude-desktop-bin/releases/latest/download/claude-desktop-bin_1.17377.1-1_amd64.deb
...
```bash
wget https://github.com/patrickjaja/claude-desktop-bin/releases/latest/download/claude-desktop-bin-1.17377.1-1.x86_64.rpm
...
wget https://github.com/patrickjaja/claude-desktop-bin/releases/latest/download/Claude_Desktop-1.17377.1-x86_64.AppImage
...
| `fix_builtin_mcp_browser_env.nim` | Adds DISPLAY/Wayland/XDG/DBUS/BROWSER to the built-in MCP env allowlist (upstream's minimal `HOME,LOGNAME,PATH,SHELL,TERM,USER` set has no display vars - fine on macOS, but on Linux an MCP server can't open a browser for OAuth) ([#139](https://github.com/patrickjaja/claude-desktop-bin/issues/139)) | `rg -o '"HOME","LOGNAME","PATH","SHELL","TERM","USER".{0,40}' index.js` |
