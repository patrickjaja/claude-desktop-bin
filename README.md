...
```bash
wget https://github.com/patrickjaja/claude-desktop-bin/releases/latest/download/claude-desktop-bin_1.14271.0-2_amd64.deb
...
```bash
wget https://github.com/patrickjaja/claude-desktop-bin/releases/latest/download/claude-desktop-bin-1.14271.0-2.x86_64.rpm
...
wget https://github.com/patrickjaja/claude-desktop-bin/releases/latest/download/Claude_Desktop-1.14271.0-x86_64.AppImage
...
| `fix_cowork_download_status_linux.nim` | Suppresses the false "Download a one-time package" agent-mode banner on Linux **native** mode (no VM image needed); leaves win/mac and Linux-KVM untouched. Gated on `process.platform==="linux"&&!globalThis.__coworkKvmMode` | `rg -o 'getDownloadStatus\(\)\{return' index.js` |
