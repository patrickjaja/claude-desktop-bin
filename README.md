...
> **You do not need the OVMF/virtiofsd symlink workaround.** Some AI-generated guides tell Arch users to symlink `/usr/share/edk2-ovmf/x64` firmware to a Debian `/usr/share/OVMF/OVMF_CODE_4M.fd` path, or `/usr/lib/virtiofsd` to `/usr/bin/virtiofsd`. This is **unnecessary on our packages (>= v1.17377.1)** - the `fix_cowork_firmware_paths_linux` patch already makes the probe search the distro-native paths (`/usr/share/edk2/x64/OVMF_CODE.4m.fd`, `/usr/lib/virtiofsd`, …). The symlinks just litter package-owned directories; skip them.
