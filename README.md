...
```bash
wget https://github.com/patrickjaja/claude-desktop-bin/releases/latest/download/claude-desktop-bin_1.569.0-12_amd64.deb
...
```bash
wget https://github.com/patrickjaja/claude-desktop-bin/releases/latest/download/claude-desktop-bin-1.569.0-12.x86_64.rpm
...
wget https://github.com/patrickjaja/claude-desktop-bin/releases/latest/download/Claude_Desktop-1.569.0-x86_64.AppImage
...
> **GNOME:** On GNOME 46+/Ubuntu 25.10+, the preferred method uses the XDG ScreenCast portal with PipeWire restore tokens — the first screenshot shows a one-time permission dialog, all subsequent screenshots are silent (requires `python3-gi`/`python-gobject` and `gst-plugin-pipewire`, typically pre-installed on GNOME). Falls back to `gdbus` (from glib2/libglib2.0-bin) and `gnome-screenshot`. The restore token is stored in `~/.config/Claude/pipewire-restore-token`.
>
...
| `fix_asar_folder_drop.py` | Prevents app.asar from being misdetected as a folder drop on launch ([#24](https://github.com/patrickjaja/claude-desktop-bin/issues/24)) | `rg -o '__cdb_isAsarPath' index.js` |
