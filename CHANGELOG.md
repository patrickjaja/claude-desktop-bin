...
- **Cowork runtime dependencies declared** as soft deps: `qemu-base` + `edk2-ovmf` (Arch optdepends), `qemu-system-x86` + `edk2-ovmf` (RPM Suggests), `qemu-system-x86` + `ovmf` (+ `qemu-efi-aarch64` on the arch-any deb). QEMU and OVMF are not bundled; Cowork needs them plus `/dev/kvm` and `kvm`-group membership.
