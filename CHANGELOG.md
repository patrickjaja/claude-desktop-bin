...
- **Version bump v1.13576.1 -> v1.13576.4 (patch-level rebuild of the same major build 13576).** Upstream `.latest` reports `1.13576.4` (hash `414f858c8545ff8af38af267a1da714429ee98f9`). Bundle re-minified but functionally identical: unpatched `index.js` 15.12 MB (15,115,730 bytes, vs 15,118,450 for v1.13576.1), vendored `@anthropic-ai/claude-agent-sdk` copies unchanged at **0.3.174 / 0.3.177** (still two copies, two match sites each).
- **All 52 patches apply unchanged; 51/52 validate via `validate-patches.sh`.** The one validator "FAIL" is `fix_ion_dist_linux` reporting "target directory not found" - a manual-extract artifact only (ion-dist lives in the MSIX resources at `app/resources/ion-dist`, not inside `app.asar`, so a bare `asar extract` doesn't produce it). Ran the ion-dist patch directly against the freshly-extracted SPA: both sub-patches apply (`org-plugins linux path`, `mount path platform ternary`); `mountPath` still ships `{mac,win}` only (no `linux` key upstream), so the patch is still required. The full `build-local.sh` (which copies ion-dist from the MSIX) built the package cleanly: `claude-desktop-bin-1.13576.4-1-x86_64.pkg.tar.zst`. `node --check` passes on both `index.js` and `index.pre.js`.
- **Unpatched platform-conditional counts vs v1.13576.1 (apples-to-apples, both raw asar extracts):** `process.platform==="linux"` **10 occurrences in both** (all third-party vendored-library checks - `signal-exit` signal lists, `is-wsl`, `linux-arm64`/`linux-x64` arch detection, `/proc/version` reads; upstream still ships **no first-party Linux platform branches** - which is why our patches exist). darwin **77 -> 77** (identical). win32 **137 -> 138** (+1 = a vendored `~`-homedir-expansion path helper `A=>process.platform==="win32"&&...test(A)`, a library internal, not a Claude feature gate - nothing actionable for Linux). Normalized fingerprint sets (identifiers stripped) are byte-for-byte identical across all three platform keys; the raw diff is pure minified-identifier churn.
- **Documented fragile anchors re-verified, all match:** enterprise wrapper `function Sei(){const A=Dei();return Object.keys(A).length>0?A:void 0}` (renamed `Mei->Sei`/`Rei->Dei`, absorbed by the flexible pattern; `readPlistValue` refs still 1), Cowork `yukonSilver:Zce()`, Cowork VM-bundle arch hardcode `const A="win32"`, tray-icon `switch(...){case"ico":...}`, feature-flag fn `dt("770567414")`.
- **No baseline-doc changes:** feature flags, built-in MCP servers (1 `registerInternalMcpServer`), ion-dist patterns (config chunk `c71860c77-BYlIO-e6.js`, content-hashed), and platform gates all unchanged. `.electron-version` stays 42.0.0 (shasums verified OK). Bumped `.upstream-version` 1.13576.1 -> 1.13576.4.
