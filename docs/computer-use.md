...
**How it works on Linux:** upstream Computer Use is macOS-only - gated behind `process.platform==="darwin"`, macOS TCC permissions, and a native Swift executor. The patch ([`fix_computer_use_linux.nim`](../patches/fix_computer_use_linux.nim)) removes the platform gates, routes both upstream executor factories to an injected Linux executor, bypasses TCC with a no-op `{granted: true}`, and auto-detects your session type to use the right tools (xdotool/ydotool, grim/spectacle/scrot/portal, plus `kwin-portal-bridge` on KDE Wayland). Install the packages for your session from your distro's [Installation](../README.md#installation) section, or see [Computer Use dependencies](computer-use-dependencies.md) for the full matrix and `ydotool` setup.
